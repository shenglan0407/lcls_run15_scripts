...
try:
    ...
    sys.exit()
out_file2 = run_file.replace('.tbl','_intershot_uncertainty.h5')
...
for qidx in qcluster_inds:
    ...
    inter_shots = denoise_Train[::2]-denoise_Train[1::2]
    ...
    f_out2.create_dataset('q%d/pca%d/random_split_err2'%(qidx,nn)
        ,data=e2)
    f_out2.create_dataset('q%d/pca%d/intershot_err'%(qidx,nn)
        ,data=intershot_difcor.std(0)/np.sqrt(inter_shots.shape[0]))
    ...
    print('not doing denoising, just computing baseline intershot_err')
    ...
    f_out2.create_dataset('q%d/pca0/intershot_err'%qidx
    ,data=difcor)
