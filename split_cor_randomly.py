...
all_files =glob.glob('%s/*PCA*.h5'%os.path.join(data_dir,sample))
print all_files
run_nums = [int(os.path.basename(ff).split('_')[0].split('n')[-1]) for ff in all_files]

outfname = os.path.join(save_dir,'%s_random_split_maskDivide.h5'%sample)
...
with h5py.File(outfname,'a') as f_out:
    for run in run_nums:
        ...
        for qidx in range(35):
            ...
            f_out.create_dataset('run%d/q%d_train_random_inds'%(run,qidx),data=a)
            x1=sorted(a[:len(a)/2])
            x2=sorted(a[len(a)/2:])

            cc = f['q%d'%qidx]['pca%d'%pca_num]['all_train_difcors'][x1,0]/mask_cor[qidx][None,:]
            ave1 = cc.mean(0)
            # std1 = cc.std(0)
            n1 = cc.shape[0]
            all_nums1.append(n1)
            ...
            cc = f['q%d'%qidx]['pca%d'%pca_num]['all_train_difcors'][x2,0]/mask_cor[qidx][None,:]
            ave2 = cc.mean(0)
            # std2 = cc.std(0)
            n2 = cc.shape[0]
            all_nums2.append(n2)
